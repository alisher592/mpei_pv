...
for elem in table1.findAll("div", {"class": "cc_0"}):
    for tag in elem.find_all(onmouseover=True):
        ...
        clouds_info_raw_text = re.findall(r'\b\w+', tag['onmouseover']) #сырая информация с детализацией по облачности
        ...
        if re.findall(r'\d{1,4}%', tag['onmouseover']) != []:
            cld.append([int(s) for s in (
                re.findall(r'\d{1,4}', tag['onmouseover']))])  # добавление значений облачности в процентах в список
...
artificial_datetime = pd.date_range(datetime.now() + timedelta(hours=1), datetime.now()+timedelta(hours=36), freq='h').strftime("%y-%m-%d %H")
